        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          <div v-for="newsItem in news" :key="newsItem.id" class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer" @click="openNewsModal(newsItem)">
            ...
            <div class="p-4 sm:p-6 text-left">
              <p class="text-xs sm:text-sm text-gray-500 mb-1 sm:mb-2">{{ formatDate(newsItem.date) }}</p>
              <h3 class="text-lg sm:text-xl font-semibold text-gray-800 mb-2 sm:mb-3">{{ newsItem.title }}</h3>
              <p class="text-sm sm:text-base text-gray-600">{{ newsItem.description }}</p>
              <span class="text-brand-accent hover:underline mt-3 sm:mt-4 inline-block">Lire la suite</span>
            </div>
          </div>
        </div>
